...
vec4 wf(int q) {
  return texelFetch(wts, ivec2(q & ${rq}, q >> ${oq}), 0);
}
...
`;function aq(n){const t=n.planes,e=n.norm==="none";return vs(n)+`uniform highp sampler3D src;
`+Ir+`uniform int uZ;
uniform int uDil;
uniform int uWQ0;
...
`):"")+Zc(t,"o","a")+`}
`}function lq(n){const t=n.planes,e=n.norm==="none",s=o=>`  { for (int i = 0; i < 4; ++i) {
      float v = q${o}[i];
      int w = b + (${o*4} + i) * P;
`+ue(t,r=>`      a${r} += wf(w + ${r}) * v;
`)+`    } }
...
`):"")+Zc(t,"o","a")+Zc(t,"n","b")+`}
`}const uq=n=>vs(n)+`uniform highp sampler3D src;
...
`,hq=n=>vs(n)+`uniform highp sampler2D pSum, pSq;
layout(location = 0) out vec4 oSum;
layout(location = 1) out vec4 oSq;
void main() {
  int x = int(gl_FragCoord.x);
...
`;function pq(n){const t=n.planes,e=s=>`  vec4 v${s} = (texelFetch(s${s}, p, 0) - texelFetch(pMean, ivec2(0, ${s}), 0))
...
`)+`}
`}function fq(n){const t=n.planes,e=ue(t,s=>s===0?`      float sv = c0[c];
...
`}function mq(n){if(n.nclass>32)throw new Error("grouped tissue masks support at most 32 classes");const t=n.planes,s=`      float acc = uHasBias != 0 ? ws(uWBias + k) : 0.0;
      for (int c = 0; c < CHAN; ++c) {
