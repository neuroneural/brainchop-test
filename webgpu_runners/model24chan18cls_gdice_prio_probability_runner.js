...
const model24chan18cls_gdice_prio_probability = (() => {
...
const createTissueParamsUniformBuf = (device, options = {}, displayOverride = null) => {
  const defaultGroups = {
    grayMatter: [2, 6, 7, 8, 9, 10, 14, 15, 16, 17],
    whiteMatter: [1, 5],
    csf: [3, 4, 11, 12],
  };
  const groups = { ...defaultGroups, ...(options.probabilityGroups || {}) };
  const maskFor = (labels) => (labels || []).reduce((mask, label) => {
    if (!Number.isInteger(label) || label < 0 || label >= 18) {
      throw new Error(`Invalid 18-class tissue label: ${label}`);
    }
    return (mask | (1 << label)) >>> 0;
  }, 0);
  const tissueCodes = { grayMatter: 0, whiteMatter: 1, csf: 2 };
  const display = displayOverride || options.probabilityDisplay || 'grayMatter';
  if (!(display in tissueCodes)) throw new Error(`Unknown probabilityDisplay: ${display}`);

  const temperature = Number(options.softmaxTemperature ?? 1);
  if (!Number.isFinite(temperature) || temperature <= 0) {
    throw new Error(`softmaxTemperature must be finite and > 0, got ${temperature}`);
  }
  const brainSupportTemperature = Number(options.brainSupportTemperature ?? 1);
  if (!Number.isFinite(brainSupportTemperature) || brainSupportTemperature <= 0) {
    throw new Error(`brainSupportTemperature must be finite and > 0, got ${brainSupportTemperature}`);
  }
  const brainSupportPower = Number(options.brainSupportPower ?? 1);
  if (!Number.isFinite(brainSupportPower) || brainSupportPower <= 0) {
    throw new Error(`brainSupportPower must be finite and > 0, got ${brainSupportPower}`);
  }
  const partialVolumeSigma = Number(options.partialVolumeSigma ?? 0);
  if (!Number.isFinite(partialVolumeSigma) || partialVolumeSigma < 0 || partialVolumeSigma > 2) {
    throw new Error(`partialVolumeSigma must be between 0 and 2 output voxels, got ${partialVolumeSigma}`);
  }

  // WGSL uniform layout (32 bytes): display temperature, tissue code, three
  // masks, support temperature/power, and Gaussian sigma. The masks keep the
  // anatomical policy in the model entry rather than in the generated runner.
  const raw = new ArrayBuffer(32);
  const floats = new Float32Array(raw);
  floats[0] = temperature;
  const words = new Uint32Array(raw);
  words[1] = tissueCodes[display];
  words[2] = maskFor(groups.grayMatter);
  words[3] = maskFor(groups.whiteMatter);
  words[4] = maskFor(groups.csf);
  floats[5] = brainSupportTemperature;
  floats[6] = brainSupportPower;
  floats[7] = partialVolumeSigma;
  ...
  new Uint8Array(buf.getMappedRange()).set(new Uint8Array(raw));
  buf.unmap();
  return { buf, partialVolumeSigma };
};
...
// Replaces only the generated argmax pass. The preceding pass still computes
// each voxel's maximum logit, which makes both softmax calculations stable.
// The display temperature controls tissue uncertainty. A separate, normally
// lower temperature computes (1 - P(background)) and stores it in the max-logit
// scratch buffer for a final soft brain-support gate.
const tissue_probability = `enable f16;
struct TissueParams {
  temperature: f32,
  tissue: u32,
  gray_matter_mask: u32,
  white_matter_mask: u32,
  csf_mask: u32,
  brain_support_temperature: f32,
  brain_support_power: f32,
  partial_volume_sigma: f32,
};
@group(0) @binding(0) var<uniform> params: TissueParams;
@group(0) @binding(1) var<storage, read_write> output_probability: array<f32>;
@group(0) @binding(2) var<storage, read_write> logits: array<f16>;
@group(0) @binding(3) var<storage, read_write> max_logits: array<f16>;

const VOXELS: u32 = 16777216u;
const THREADS_X: u32 = 8388608u;

@compute @workgroup_size(256)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  let voxel = gid.x + gid.y * THREADS_X;
  if (voxel >= VOXELS) { return; }

  var selected_mask = params.gray_matter_mask;
  if (params.tissue == 1u) { selected_mask = params.white_matter_mask; }
  if (params.tissue == 2u) { selected_mask = params.csf_mask; }

  let inverse_temperature = 1.0 / max(params.temperature, 0.000001);
  let inverse_support_temperature = 1.0 / max(params.brain_support_temperature, 0.000001);
  let maximum = f32(max_logits[voxel]);
  var denominator = 0.0;
  var numerator = 0.0;
  var support_denominator = 0.0;
  var background_weight = 0.0;
  for (var channel = 0u; channel < 18u; channel++) {
    let centered_logit = f32(logits[channel * VOXELS + voxel]) - maximum;
    let value = exp(centered_logit * inverse_temperature);
    let support_value = exp(centered_logit * inverse_support_temperature);
    denominator += value;
    support_denominator += support_value;
    if (channel == 0u) { background_weight = support_value; }
    if (((selected_mask >> channel) & 1u) != 0u) { numerator += value; }
  }
  output_probability[voxel] = numerator / denominator;
  let brain_support = 1.0 - background_weight / support_denominator;
  max_logits[voxel] = f16(clamp(brain_support, 0.0, 1.0));
}`;

const make_gaussian_blur = (axis) => `enable f16;
struct TissueParams {
  temperature: f32,
  tissue: u32,
  gray_matter_mask: u32,
...
@group(0) @binding(0) var<uniform> params: TissueParams;
@group(0) @binding(1) var<storage, read_write> output_probability: array<f32>;
@group(0) @binding(2) var<storage, read_write> input_probability: array<f32>;
@group(0) @binding(3) var<storage, read_write> support_scratch: array<f16>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let x = i32(voxel & 255u);
  let y = i32((voxel >> 8u) & 255u);
  let z = i32(voxel >> 16u);
  let sigma = max(params.partial_volume_sigma, 0.000001);
  let radius = max(1, i32(round(3.0 * sigma)));
  ...
  for (var offset = -6; offset <= 6; offset++) {
    ...
    var sy = y;
    var sz = z;
    if (AXIS == 0u) { sx += offset; }
    if (AXIS == 1u) { sy += offset; }
    if (AXIS == 2u) { sz += offset; }
    if (sx < 0 || sy < 0 || sz < 0 || sx >= 256 || sy >= 256 || sz >= 256) { continue; }
    let sample_voxel = u32(sx + sy * 256 + sz * 65536);
    weighted_sum += weight * input_probability[sample_voxel];
  }
  output_probability[voxel] = weighted_sum / weight_sum;
}`;

const apply_brain_support = `enable f16;
struct TissueParams {
  temperature: f32,
  tissue: u32,
  gray_matter_mask: u32,
...
@group(0) @binding(1) var<storage, read_write> output_probability: array<f32>;
@group(0) @binding(2) var<storage, read_write> input_probability: array<f32>;
@group(0) @binding(3) var<storage, read_write> brain_support: array<f16>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  let voxel = gid.x + gid.y * THREADS_X;
  if (voxel >= VOXELS) { return; }
  let support = pow(clamp(f32(brain_support[voxel]), 0.0, 1.0), params.brain_support_power);
  output_probability[voxel] = input_probability[voxel] * support;
}`;

const setupNet = async (device, safetensor, _callbackUI, options = {}) => {
    const metadata = getTensorMetadata(safetensor);
    const infinityBuf = createInfinityUniformBuf(device);
    const catLite = options.probabilityPostprocess === 'cat-lite';
    const tissueParamSets = (catLite
      ? ['grayMatter', 'whiteMatter', 'csf']
      : [options.probabilityDisplay || 'grayMatter']
    ).map((display) => createTissueParamsUniformBuf(device, options, display));
    const { buf: tissueParamsBuf, partialVolumeSigma } = tissueParamSets[0];
    ...
    const postprocessPipelines = await Promise.all([
      make_gaussian_blur(0),
      make_gaussian_blur(1),
      make_gaussian_blur(2),
      apply_brain_support,
    ].map(async (code) => {
      return await device.createComputePipelineAsync({
        layout: device.createPipelineLayout({ bindGroupLayouts: [layouts[106]] }),
    ...
    return async (_input0) => {
        ...
        if (catLite) {
          // Keep the 18 logits alive and read GM, WM and CSF sequentially. This
          // costs no additional full-volume GPU buffers: output0, the dead first
          // activation buffer and the single readback buffer are reused for all
          // three tissues. CAT-lite performs its joint PVE refinement on CPU.
          device.queue.submit([commandEncoder.finish()]);
          const tissueResults = [];
          for (const { buf: paramsBuf } of tissueParamSets) {
            const tissueEncoder = device.createCommandEncoder();
            // tissue_probability stores brain support in its third storage
            // buffer. Preserve buf_49 (the maximum logits) and use the now-dead
            // input buffer as per-tissue scratch; otherwise GM extraction
            // replaces the stabilising maxima and WM/CSF softmax can overflow.
            tissueEncoder.copyBufferToBuffer(buf_49, 0, input0, 0, buf_49.size);
            addComputePass(device, tissueEncoder, pipelines[106], layouts[106], paramsBuf, [output0, buf_46, input0], [32768, 2, 1]);
            let tissueOutput = buf_0;
            if (partialVolumeSigma > 0) {
              // Smooth each tissue PRIOR with the same scanner-PSF kernel before
              // CAT-lite jointly renormalizes and fits the mixed tissue classes.
              // This never destroys the logits in buf_46, which are needed to
              // derive the next tissue map.
              addComputePass(device, tissueEncoder, postprocessPipelines[0], layouts[106], paramsBuf, [buf_0, output0, input0], [32768, 2, 1]);
              addComputePass(device, tissueEncoder, postprocessPipelines[1], layouts[106], paramsBuf, [output0, buf_0, input0], [32768, 2, 1]);
              addComputePass(device, tissueEncoder, postprocessPipelines[2], layouts[106], paramsBuf, [buf_0, output0, input0], [32768, 2, 1]);
              addComputePass(device, tissueEncoder, postprocessPipelines[3], layouts[106], paramsBuf, [output0, buf_0, input0], [32768, 2, 1]);
              tissueOutput = output0;
            } else {
              addComputePass(device, tissueEncoder, postprocessPipelines[3], layouts[106], paramsBuf, [buf_0, output0, input0], [32768, 2, 1]);
            }
            tissueEncoder.copyBufferToBuffer(tissueOutput, 0, gpuReadBuffer0, 0, output0.size);
            device.queue.submit([tissueEncoder.finish()]);
            await gpuReadBuffer0.mapAsync(GPUMapMode.READ);
            const tissueResult = new Float32Array(gpuReadBuffer0.size / 4);
            tissueResult.set(new Float32Array(gpuReadBuffer0.getMappedRange()));
            gpuReadBuffer0.unmap();
            tissueResults.push(tissueResult);
          }
          return tissueResults;
        }
        addComputePass(device, commandEncoder, pipelines[106], layouts[106], tissueParamsBuf, [output0, buf_46, buf_49], [32768, 2, 1]);
        let finalOutput = buf_46;
        if (partialVolumeSigma > 0) {
          // Reuse the now-dead logit buffer as a float scratch volume, so the
          // three separable blur passes add no peak storage allocation.
          addComputePass(device, commandEncoder, postprocessPipelines[0], layouts[106], tissueParamsBuf, [buf_46, output0, buf_49], [32768, 2, 1]);
          addComputePass(device, commandEncoder, postprocessPipelines[1], layouts[106], tissueParamsBuf, [output0, buf_46, buf_49], [32768, 2, 1]);
          addComputePass(device, commandEncoder, postprocessPipelines[2], layouts[106], tissueParamsBuf, [buf_46, output0, buf_49], [32768, 2, 1]);
          finalOutput = output0;
          addComputePass(device, commandEncoder, postprocessPipelines[3], layouts[106], tissueParamsBuf, [finalOutput, buf_46, buf_49], [32768, 2, 1]);
        } else {
          addComputePass(device, commandEncoder, postprocessPipelines[3], layouts[106], tissueParamsBuf, [finalOutput, output0, buf_49], [32768, 2, 1]);
        }
        commandEncoder.copyBufferToBuffer(finalOutput, 0, gpuReadBuffer0, 0, output0.size);
        device.queue.submit([commandEncoder.finish()]);
...
export default model24chan18cls_gdice_prio_probability;
