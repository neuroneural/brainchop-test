...
const createTissueParamsUniformBuf = (device, options = {}) => {
  const defaultGroups = {
  ...
  const display = options.probabilityDisplay || 'grayMatter';
...
const setupNet = async (device, safetensor, _callbackUI, options = {}) => {
    ...
    const { buf: tissueParamsBuf, partialVolumeSigma } = createTissueParamsUniformBuf(device, options);
