// Backbone shaders/dispatches match the accelerated 24-channel label runner.
// CAT-lite keeps its own probability tail and uses the dead classifier-input
// buffer for blur scratch, preserving the aliased logits for all three tissues.
...
const setupNet = async (device, safetensor, _callbackUI, options = {}) => {
    ...
    // Port the accelerated label runner's lifetime reuse: buf_6 is last
    // read at pass 96, and the classifier input starts at pass 103.
    const buf_43 = buf_6;
    ...
    // Pass 103 consumes buf_0 before pass 104 writes the classifier logits.
    // Keep these logits alive through all three CAT-lite tissue extractions.
    const buf_46 = buf_0;
    ...
    return async (_input0) => {
        ...
        if (catLite) {
          ...
          // costs no additional full-volume GPU buffers: output0, the dead
          // classifier-input buffer (buf_43 / buf_6), and the readback buffer
          // are reused for all three tissues. Joint PVE refinement runs on CPU.
          ...
          for (const { buf: paramsBuf } of tissueParamSets) {
            ...
            if (partialVolumeSigma > 0) {
              ...
              addComputePass(device, tissueEncoder, postprocessPipelines[0], layouts[106], paramsBuf, [buf_6, output0, input0], [32768, 2, 1]);
              addComputePass(device, tissueEncoder, postprocessPipelines[1], layouts[106], paramsBuf, [output0, buf_6, input0], [32768, 2, 1]);
              addComputePass(device, tissueEncoder, postprocessPipelines[2], layouts[106], paramsBuf, [buf_6, output0, input0], [32768, 2, 1]);
              addComputePass(device, tissueEncoder, postprocessPipelines[3], layouts[106], paramsBuf, [output0, buf_6, input0], [32768, 2, 1]);
              tissueOutput = output0;
            } else {
              addComputePass(device, tissueEncoder, postprocessPipelines[3], layouts[106], paramsBuf, [buf_6, output0, input0], [32768, 2, 1]);
            }
